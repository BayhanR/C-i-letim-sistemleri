...
int main() {
    DoublyLinkedList processTable = { NULL, NULL };
    insertNode(&processTable, 1001, (PCB){.program_counter = 0});
    insertNode(&processTable, 1002, (PCB){.program_counter = 10});
    insertNode(&processTable, 1003, (PCB){.program_counter = 20});
    terminateProcess(&processTable ,1002);
    listProcess(&processTable);
    return 0;
}
